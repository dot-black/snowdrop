...
class ManagerStoriesTest < ActionDispatch::IntegrationTest
  ...
  test "check discounts" do
    ...
    get "/manager/discounts/new"
    assert_response :success
    assert_template "new"

    post "/manager/discounts", params:{
      discount:{
        title: "New discount",
        value: 10
      }
    ...
    user = Discount.last
    get "/manager/discounts/#{user.id}"
    assert_response :success
    assert_template "show"

    get "/manager/discounts/#{user.id}/edit"
    assert_response :success
    assert_template "edit"

    put "/manager/discounts/#{user.id}", params:{
      discount:{
        title: "Updated title",
        value: 20
      }
    }
    assert_redirected_to manager_discount_path(user)

    get "/manager/discounts/#{user.id}/change_appearance"
    assert_redirected_to manager_discount_path(user)

    get "/manager/discounts/#{user.id}/edit_products"
    assert_response :success
    assert_template "edit_products"

    put "/manager/discounts/#{user.id}/update_products", params:{
      product_ids: Product.all.ids
    }
    assert_redirected_to edit_products_manager_discount_path(user)
